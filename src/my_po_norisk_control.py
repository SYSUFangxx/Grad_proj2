...
def trade(context, bar_dict):
    ...
    good_stocks = list(set(GOOD_CODES_YEAR['2017'] + GOOD_CODES_YEAR['2018'] + GOOD_CODES_YEAR['2019']))

    # 持仓股票
    position_stocks = list(context.portfolio.positions.keys())

    # 总股池股票，包含基准成分股、持仓股票，且有因子数据、不在去除取票序列中的股票
    all_stocks = (set(ben_stocks) | set(position_stocks)) & set(df_X.index) - set(exclude_stocks)
    ...
    index_good_stock = list(set(good_stocks) & set(df_X.index) & set(all_stocks) - set(exclude_stocks))
