...
def trade(context, bar_dict):
    ...
    # else:
    #     x_pred = 1 / p.mean()
    # 文章中有对应的求解算法，这里作便捷考虑，直接采用mean函数计算
    ...
    po_inputs = {'w_o': w_o, 'x_pred': x_pred.values, 'eps': 0}

    if context.init_flag:
        weights = np.ones(len(po_stocks)) / len(po_stocks)
        context.init_flag = False
    else:
        weights = ReferPO.rmr(po_inputs)

    print("Nonzero weights", [(s, w) for s, w in zip(po_stocks, weights) if w])
